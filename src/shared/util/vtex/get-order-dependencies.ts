import { ContentDependencies, ContentManifest, DependenciesListModel, logger, ObjLevelImportance } from "../../../shared";
...
export const resolverDependencies = async (
  ...
  for (const prop in dependencies) {
    let manifest = manifests.find(res => {
      if (
        ...
        logger.debug(`dependencies found: ${prop}`);
    ...
    if (manifest) {
      resolverDependencies(manifest.dependencies, app_name);
    }
  }
};
