...
- Add new repo for clone on run `exito init`

### v2.8.0 2019-11-19

## Changed

- Create shared folder for export all resources from `models` `utils`.
- Create new command for execute one command in the projects selected

### v2.7.8 2019-11-05

## Added

- Add command to execute the triggers `put` or `update`

### v2.7.6 2019-07-24

## Added

- Change validation to generate versions, add `--last` validation

- Added logic for scape the command passed on `link` or `publish` in vtex, this option is for AWS CodeBuild process.

- Added `exito vtex updateDepProject` command

### v2.7.0 2019-07-22

## Changed

- Add optional arg to the command exito run overWriteDependencies, this option indicate if only use the last version from one dependencie or not
